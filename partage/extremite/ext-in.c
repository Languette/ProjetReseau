...
int main(int argc, char *argv[])
{
  ...
  if (argc!=4) {/* erreur de syntaxe */
    printf("Usage: %s hote port addresse_d'interface_tun0\n",argv[0]);
    exit(1);
  }
  char commands[40];
  printf("Création de %s\n",argv[3]);
  tunfd = tun_alloc(argv[3]);
  sprintf(commands, "ip addr add %s dev tun0", argv[3]);
  system(commands);

  sprintf(commands, "ip link set dev tun0 up");
  system(commands);
